...
export const KLEDO_QUERY_FILTER_COMPATIBILITY = [
  ...
  'in is allowed only for productId on its supported transaction entities, product.categoryId, bank_transaction.transactionType, sales_invoice.salesPersonId, sales_order.statusId (status_id for eq; status_ids for in), and warehouseId on purchase_invoice, sales_order, purchase_order, purchase_delivery, sales_quote, or purchase_quote.',
  'sales_invoice=contactId,statusId,productId,salesPersonId,transactionDate,dueDate,shippingDate,paymentDate,amount;',
